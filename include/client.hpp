...
public:
  TcpClient(const std::string &host, const std::string &port) {
    ...
    int connection_status =
        connect(connection_port, res->ai_addr, res->ai_addrlen);
  ...
  void sendData(const std::string &text) { m.sendData(text, connection_port); }

  std::string receiveData() { return m.receiveData(connection_port); }
