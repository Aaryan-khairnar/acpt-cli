...
client attempts to send message:- It will encode header and put message into the
data and send data server gets the message, decodes the header, get the
bodylength from the header and hence complete body then server sends the message
to all the clients connected to that room.
...
public:
  ...
  void sendData(const std::string &text, int &port) {
    ...
    while (totalSent < toSend) {
      ssize_t sent =
          send(port, buffer.data() + totalSent, toSend - totalSent, 0);
      if (sent <= 0) {
  ...
  std::string receiveData(int &port) {
    ...
    int headerValue = (headerBuf[0] - '0') * 1000 + (headerBuf[1] - '0') * 100 +
                      (headerBuf[2] - '0') * 10 + (headerBuf[3] - '0');
